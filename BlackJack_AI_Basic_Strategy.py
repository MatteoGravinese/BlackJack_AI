...
def BettingRules():
    global AIMoney, OriginalBet, RiskFactor, AmountBet, HandsPlayed, HandNumber, TurnOver, BettingClosed, Win
    if 10<AIMoney<EndingMoney:
        if Win==1:
            if AIMoney>=OriginalBet:
                AmountBet=OriginalBet
            else:
                AmountBet=AIMoney/(2**RiskFactor)
        elif HandNumber==0:
            AmountBet=OriginalBet
        elif Win==0 and AIMoney>=2*AmountBet:
