...
def BettingRules():
    global AIMoney, RiskFactor, AmountBet, HandsPlayed, HandNumber, TurnOver, BettingClosed, Win, LossTracker
    if 10<AIMoney<EndingMoney:
        if Win==1 or HandNumber==0:
            AmountBet=AIMoney/(2**RiskFactor)
        elif Win==0 and LossTracker<3:
    ...
    if AIMoney<0:
        print(AIMoney, AmountBet, AINumber)
...
while AINumber<TotalAINumber:
    ...
    while HandNumber<HandsPlayed:
        ...
        DealerHand.sort()
        if DealerHand[0]==1 and BettingClosed==0:
            if DealerHand[1]==10:
                if AIHand[0]==1 and AIHand[1]>9:
                    Win=2
                    AIMoney=AIMoney+AmountBet
